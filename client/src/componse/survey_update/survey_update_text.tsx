...
export const Edit_text_row = () => {
    ...
    const [text_check, setText_check] = useState(state.will_editing_data.check)


    const [model_show, setModel_show] = useState(false)
    const [fileList, setFile_picture] = useState(state.will_editing_data.picture)
    ...
    const check = () => {
        ...
        if (question_text !== "") {
            const data = { question_text, is_require, id: state.will_editing_data.id, question_type: type_quesition.text, check: text_check, picture: fileList }
