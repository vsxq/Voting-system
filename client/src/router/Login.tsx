...
import { useInputValue, dynamicJs, dynamicCss } from "../utils/base_utils"
import { ajax_post,ajax_get } from "../utils/ajax"
...
export const Login = () => {
  dynamicJs("/js/MorphSVGPlugin.min.js")
  dynamicJs("/js/index.js")
  dynamicCss("/css/reset.min.css")
  dynamicCss("/css/style.css")
  const [toast_info, setToast_info] = useState({ "status": false, "note": "" })
  ...
  const toast = (note: string): void => {
    setToast_info({ "status": true, "note": note })

    setTimeout(() => { setToast_info({ ...toast_info, "status": false }) }, 2000)
  ...
  const check = async () => {
    ...
    if (user.value !== "" && passwd.value !== "") {
      ...
      const data: any = await ajax_get(login_url, { user: user.value, passwd: passwd.value })
      switch (data.status) {
        case status_enum.success_info: {
          localStorage.setItem("jwt", data.token)
          localStorage.setItem("login_time", data.date)
          localStorage.setItem("username",user.value)
          localStorage.setItem("level",data.level)
          window.location.href="/manage"
        }; break;
        case status_enum.login_invalid_user: { toast("账号不存在"); user.setValue("") }; break;
        case status_enum.login_invalid_passwd: { toast("密码错误"); passwd.setValue("") }; break;
        default: { toast("可能服务器开小差了") }; break
      }
    }
    else {
      toast("请检查输入")
    ...
    <>
      <form>
        ...
        <div className="inputGroup inputGroup1" >
          ...
          <input type="text" id="user" className="email" maxLength={256} value={user.value} onChange={user.onChange} required pattern="[A-Za-z0-9]+" />
          <p className="helper helper1">由字母或数字组成</p>
          <span className="indicator" />
        </div>
        <div className="inputGroup inputGroup2">
          <label htmlFor="password" >密码</label>
          <input type="password" id="password" className="password" value={passwd.value} onChange={passwd.onChange} required pattern="[A-Za-z0-9]+" />
        </div>
        <div className="inputGroup inputGroup3">
          <button type="button" id="login" onClick={check} value="log in" >log in</button>
        </div>
        <div className=" weui-toptips " style={{ display: (toast_info.status === true) ? "block" : "none" }}>{toast_info.note}</div>
